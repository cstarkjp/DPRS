...
use super::{Cell3D, CellModel, CellNbrhood3D, LatticeDualState3D};
...
fn test_3d_sim() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (history_len, lattices, _tracking) =
        simulation_nd::<ChaCha8Rng, Cell3D, LatticeDualState3D<MoveDownRightModel3D>>(
            &parameters,
        )?;
...
fn test_3d_run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_time, history_len, lattices, _tracking) =
        run_nd::<ChaCha8Rng, Cell3D, LatticeDualState3D<MoveDownRightModel3D>>(&parameters)?;
    assert_eq!(history_len, parameters.n_iterations + 1);
