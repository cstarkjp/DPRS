...
use super::{Cell1D, CellModel, LatticeModel1D, ModelSimplifiedDK1D, ModelStaggeredDK1D};
...
fn test_1d_run_random_staggered_critical() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_time, history_len, _lattices, tracking) =
        run_nd::<ChaCha8Rng, Cell1D, LatticeModel1D<ModelStaggeredDK1D>>(&parameters)?;
...
fn test_1d_run_random_simplified_critical() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_time, history_len, _lattices, tracking) =
        run_nd::<ChaCha8Rng, Cell1D, LatticeModel1D<ModelSimplifiedDK1D>>(&parameters)?;
    assert_eq!(history_len, 2);
