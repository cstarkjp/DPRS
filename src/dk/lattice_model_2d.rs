...
impl<C: CellModel<Cell2D>> LatticeModel2D<C> {
    ...
    pub fn new(cell_model: C, p: SimParameters) -> Self {
        Self {
            cell_model,
            lattice: vec![DualState::default(); p.n_x * p.n_y],
            p,
        }
    }
    ...
    fn lattice_row_mut(&mut self, y: usize) -> &mut [DualState] {
        &mut self.lattice[(y * self.p.n_x)..((y + 1) * self.p.n_x)]
    }
    ...
    pub fn create_seeded_lattice(&mut self) {
        ...
        let i = self.i_cell(self.p.n_x / 2, self.p.n_y / 2);
    ...
    pub fn apply_edge_topology(&mut self) {
        ...
        if self.p.axis_topology_x.is_periodic() {
            for row in self.lattice.chunks_exact_mut(self.p.n_x) {
                row[self.p.n_x - 2] = row[0];
                row[1] = row[self.p.n_x - 1];
            }
        }
    ...
    fn make_axis_periodic_y(&mut self, y_from: usize, y_to: usize) {
        ...
        let src = src_left..(src_left + self.p.n_x);
    ...
    pub fn apply_boundary_conditions(&mut self) {
        ...
        if self.p.axis_bcs_y.1.is_pinned() {
            ...
            self.lattice_row_mut(self.p.n_y - 1).fill(v);
        }
    }
    ...
    fn is_in_bounds_xy(&self, x: usize, y: usize) -> bool {
        x > 0 && y > 0 && x < self.p.n_x - 1 && y < self.p.n_y - 1
    }
    ...
    pub fn next_iteration_parallel<R: Rng + Send>(&mut self, rngs: &mut [R]) {
        ...
        let n_rows = self.p.n_y - 2;
        updated_lattice
            .par_chunks_mut(self.p.n_x)
            .enumerate()
            .zip(rngs)
            .skip(1)
            .take(n_rows)
    ...
    pub fn update_row<R: Rng>(&self, rng: &mut R, y: usize, row: &mut [DualState]) {
        ...
        let i_up = i_md + self.p.n_x;
        let i_dn = i_md - self.p.n_x;
